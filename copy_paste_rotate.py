...
def encode_segmap(mask_instance, mask_affordance):
    ...
    label_mask_instance = label_mask_instance.astype(np.uint8)
...
def img_add(img_src, img_main, mask_src):
    ...
    # print(mask_02.shape)
    #剔除另一张图片中mask对应的区域，归为o,为sub_img01弄出位置
    ...
    # pic2 = Image.fromarray(np.uint8(sub_img02))
...
def copy_paste(mask_src_instance, mask_src_affordance, img_src, mask_main_instance, mask_main_affordance, img_main):
    # 随机翻转,一张图片对应着instance 和 affordance
    # mask_src_instance, img_src = random_flip_horizontal(mask_src_instance, img_src)
    # mask_src_affordance, img_src = random_flip_horizontal(mask_src_affordnce, img_src)
    #
    # mask_main_instance, img_main = random_flip_horizontal(mask_main_instance, img_main)
    # mask_main_affordance, img_main = random_flip_horizontal(mask_main_affordance, img_main)
    #
    #
    #
    ...
    #     mask_src_instance, img_src = rescale_src(mask_src_instance, img_src, h, w)
    #     mask_src_affordnce, _ = rescale_src(mask_src_affordance,img_src,h, w)
...
def main(args, d):
    ...
    for mask_instance_path in tbar:
        ...
        # 指定一个原图和mask图，get source mask and img
        ...
        # 得到mask模板w*h
        mask_src_instance, mask_src_affordance = encode_segmap(mask_src_instance, mask_src_affordance)
        ...
        mask_main_instance, mask_main_affordance = encode_segmap(mask_main_instance, mask_main_affordance)
        ...
        try:
            ...
            img_rec = img.copy()
            cv2.rectangle(img_rec, (xy1[0],xy1[1]), (xy1[2],xy1[3]),[0,255,0])
            cv2.rectangle(img_rec, (xy2[0], xy2[1]), (xy2[2], xy2[3]), [0, 255, 0])
...
def get_args():
    ...
    parser.add_argument("--input_dir", default="/home/wzl/code/paper-code/paper-affordance-pose/affrodance-paper/data-generation/copy-paste-summary/dataset-generation", type=str,
                        help="input annotated directory")
    parser.add_argument("--output_dir", default="/home/wzl/code/paper-code/paper-affordance-pose/affrodance-paper/data-generation/copy-paste-summary/datasets", type=str,
